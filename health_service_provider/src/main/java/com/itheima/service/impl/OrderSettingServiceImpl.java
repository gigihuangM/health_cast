    ...
    public void add(List<OrderSetting> list) {
        if(list != null && list.size() > 0){
            for (OrderSetting orderSetting : list) {
                //判断当前日期是否已经进行了预约设置
//                String orderDate = getDateStr(orderSetting);
                Date d  = orderSetting.getOrderDate();
                long countByOrderDate = orderSettingDao.findCountByOrderDate(getDateStr(d));
...
//        String end = date + "-31";//2019-6-31
    ...
    public void editNumberByDate(OrderSetting orderSetting) {
        //根据日期查询是否已经进行了预约设置
//        String orderDate = getDateStr(orderSetting);
        long count = orderSettingDao.findCountByOrderDate(orderSetting.getOrderDateStr());
        if(count > 0){
            //当前日期已经进行了预约设置，需要执行更新操作
            orderSetting.setOrderDateStr(orderSetting.getOrderDateStr());
            orderSettingDao.editNumberByOrderDate(orderSetting);
        }else{
